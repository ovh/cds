...
export class WorkflowService {
    ...
    findNodeInWorkflowById(id: number, workflow: Workflow): WorkflowNode {
      return this.findNode(id, workflow.root, workflow.joins);
    }

    findNode(id: number, node: WorkflowNode, joins: WorkflowNodeJoin[]): WorkflowNode {
        let nodeFound;
        if (!node) {
            return;
        }
        ...
        if (Array.isArray(node.triggers) && node.triggers.length) {
            for (let n of node.triggers) {
                nodeFound = this.findNode(id, n.workflow_dest_node, joins);
                if (nodeFound) {
                    return nodeFound;
                }
            }
        ...
        if (!nodeFound && Array.isArray(joins) && joins.length) {
            for (let join of joins) {
                ...
                for (let tr of join.triggers) {
                    nodeFound = this.findNode(id, tr.workflow_dest_node, []);
                    if (nodeFound) {
                        return nodeFound;
                    }
                }
    ...
    findJoinInWorkflowById(id: number, workflow: Workflow): WorkflowNodeJoin {
