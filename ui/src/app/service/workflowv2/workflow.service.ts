...
import { V2WorkflowRun, V2WorkflowRunJob, V2WorkflowRunJobsRequest, V2WorkflowRunManualRequest, V2WorkflowRunManualResponse, WorkflowRunInfo, WorkflowRunResult } from "../../../../libs/workflow-graph/src/lib/v2.workflow.run.model";
...
export class V2WorkflowRunService {
    ...
    /**
     * Start multiple jobs in a single batch call.
     * Maps to POST /v2/project/{projectKey}/run/{workflowRunID}/job
     * with body { job_inputs: { [jobName]: { [inputName]: value } } }.
     */
    startJobs(projKey: string, workflowRunID: string, data: V2WorkflowRunJobsRequest): Observable<V2WorkflowRun> {
        return this._http.post<V2WorkflowRun>(`/v2/project/${projKey}/run/${workflowRunID}/job`, data);
    }
