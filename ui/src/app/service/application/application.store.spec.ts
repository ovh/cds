...
describe('CDS: application Store', () => {
    ...
    it('should add then delete a hook', async( () => {
        let call = 0;
        // Mock Http application request
        backend.connections.subscribe(connection => {
            switch (call) {
        ...
        a.repositories_manager.name = 'repoman';
        a.repository_fullname = 'repo';

        let h: Hook = new Hook();
        h.pipeline = new Pipeline();
        h.pipeline.name = 'pipName';

        applicationStore.addHook(proj, a, h).subscribe(() => {});
        expect(call).toBe(2, 'Need to have done 2 http call');

        // check get application
        let checkedAddHook = false;
        applicationStore.getApplications(proj.key, 'myApplication').subscribe( apps => {
            ...
            expect(apps.get(proj.key + '-myApplication').hooks.length).toBe(1, 'A hook must have been added');
            checkedAddHook = true;
        }).unsubscribe();
        expect(checkedAddHook).toBeTruthy('Need application to be updated');
        expect(call).toBe(2, 'Need to have done 2 http call');

        applicationStore.removeHook(proj, a, h).subscribe(() => {});
        expect(call).toBe(3, 'Need to have done 3 http call');

        // check get application
        let checkedDeleteHook = false;
        applicationStore.getApplications(proj.key, 'myApplication').subscribe( apps => {
            expect(apps.get(proj.key + '-myApplication').last_modified).toBe(789, 'App lastModified date must have been updated');
            expect(apps.get(proj.key + '-myApplication').hooks).toBeFalsy('App must not have pollers');
            checkedDeleteHook = true;
        }).unsubscribe();
        expect(checkedDeleteHook).toBeTruthy('Need application to be updated');
        expect(call).toBe(3, 'Need to have done 3 http call');
