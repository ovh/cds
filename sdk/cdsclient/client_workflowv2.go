...
func (c *client) WorkflowV2RunJob(ctx context.Context, projKey, vcsIdentifier, repoIdentifier, wkfName, jobIdentifier string, runNumber int64) (*sdk.V2WorkflowRunJob, error) {
	var runJob sdk.V2WorkflowRunJob
	path := fmt.Sprintf("/v2/project/%s/vcs/%s/repository/%s/workflow/%s/run/%d/jobs/%s", projKey, url.PathEscape(vcsIdentifier), url.PathEscape(repoIdentifier), wkfName, runNumber, jobIdentifier)
