...
Basically, a worker is an executable based on Docker image (or openstack, ...) that have your own tools installed, plus some tools required by **CDS**

## Worker cycle of life

* The worker is launched in the docker image (or openstack, ...). So, first, the docker must be able to download it.
* Then your stage script is executed.

As a consequence, the worker model (Docker image) must:

* have a sane installation of `curl`
* specify a working dir on which the current user is allowed to write
