...
* At the bottom of the page, fill the form
    ...
    * **command** : is the main worker command, typically on a unix system `rm -f worker && curl {{.API}}/download/worker/linux/$(uname -m) -o worker && chmod +x worker && exec ./worker` to see all the available variables to interpolate for your worker parameters [click here]({{< relref "workflows/pipelines/requirements/worker-model/variables.md" >}})
