...
* At the bottom of the page, fill the form
    ...
    * **command**: is the main worker command, typically on a Unix system `curl {{.API}}/download/worker/linux/$(uname -m) -o worker --retry 10 --retry-max-time 120 -C - && chmod +x worker && exec ./worker` to see all the available variables to interpolate for your worker parameters [click here]({{< relref "/manual/concepts/worker-model/variables.md" >}})
