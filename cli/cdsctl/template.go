...
func templatePushRun(v cli.Values) error {
	...
	if readFromLink {
		...
		if len(filesToRead) == 0 {
			return cli.NewError("wrong usage: you should specify your workflow template YAML files. See %s template push --help for more details", os.Args[0])
		}
...
func templateDetachRun(v cli.Values) error {
	...
	if wk.TemplateInstance == nil {
		return cli.NewError("given workflow is was not generated by a template")
	}
