...
func (actPlugin *runActionDownloadArtifactlugin) perform(ctx context.Context, name, path string) error {
	...
	response, err := grpcplugins.GetV2RunResults(ctx, &actPlugin.Common, workerruntime.V2FilterRunResult{Pattern: name, Type: []string{sdk.V2WorkflowRunResultTypeCoverage, sdk.V2WorkflowRunResultTypeGeneric}, WithClearIntegration: true})
	if err != nil {
		return err
	}
