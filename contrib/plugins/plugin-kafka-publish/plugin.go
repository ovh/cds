...
func (m KafkaPlugin) Description() string {
	...
	This action is a CDS Plugin packaged as a single binary file you can download and use to listen and consume data comming from CDS through Kafka. CDS can also wait for an acknowledgement comming from the consummer side. To send the acknowledgement, you can again use the plugin binary. For more details, see readme file of the plugin.`
}
...
func (m KafkaPlugin) Parameters() plugin.Parameters {
	...
	params.Add("kafkaPassword", plugin.StringParameter, "Kafka User", "{{.cds.proj.kafkaPassword}}")
