...
parameters = {
    ...
    "raw_output_file" = {
        type = "string"
        description = "File to output the raw result if raw output is checked. If empty, will only log the results."
    }
    "module_path" = {
        type = "string"
        description = "List of modules paths (absolute or relative) to launch pylint into, separated by a ;.
If empty, will launch pylint inside the working directory"
    }
    "ignore" = {
        type = "string"
        description = "List of ignored files / directory (base name, not path), separated
by a ;"
    }
    "pylintrc" = {
        type = "string"
...
def get_lint_result(module_path=None, ignore=None, pylintrc=None, extra_options=None, raw_output=False):
    """
    ...
    :rtype: list
    """
    # If pylintrc points to a file, use it, else write a temporary file to store its value.
    rcfile_path = None
    ...
    # If the user didn't provide module path(s), launch pylint in the working directory
    if not module_path:
        if os.path.isfile("__init__.py"):
            LOG.debug("Using current directory as module")
            module_path = ["."]
        else:
            LOG.error("The current working directory is not a valid python module, please fill the "
    ...
    for module in module_path:
        ...
        pylint_stdout, pylint_stderr = epylint.py_run(" ".join(pylint_options), return_std=True)
        ...
        # Parse the pylint result, if it fails, it means the pylint run failed too.
        if not raw_output:
            try:
...
def lint(xml_output_file="pylint.xml", raw_output=False, raw_output_file="pylint.txt", **kwargs):
    """
    Create a xunit file, compatible with CDS Junit parsing with the collected pylint results
    ...
    if raw_output:
        ...
        with open(xml_output_file, "w") as xml_f:
            xml_f.write('<testsuite errors="0" failures="0" name="Fake xunit test suite" skip="0" tests="0">')
    ...
    for line in lint_result:
        # If the message is on multiple lines, only set the title to the first line,
        # the following lines will be displayed inside the body.
        # An arrow (->) will be added to right side of the title to indicate that the rest of the information
        ...
        testcase = ET.SubElement(root, "testcase", classname=line["module"], name=msg, time="0.000")
...
if __name__ == '__main__':
    ...
    kwargs = {
        ...
        "raw_output": "{{.raw_output}}" in ["True", "true", "TRUE", "t", "y", "T", "Y"],
