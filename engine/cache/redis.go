...
func NewRedisStore(ctx context.Context, redisConf sdk.RedisConf, ttl int) (*RedisStore, error) {
	...
	//redis.SetLogger(log.CurrentLogger())
...
func (s *RedisStore) Get(ctx context.Context, key string, value interface{}) (bool, error) {
	if s.Client == nil {
		return false, sdk.WithStack(fmt.Errorf("redis> cannot get redis client"))
	}

	val, errRedis := s.Client.Get(ctx, key).Result()
...
func (s *RedisStore) SetWithTTL(ctx context.Context, key string, value interface{}, ttl int) error {
	if s.Client == nil {
	...
	if err := s.Client.Set(ctx, key, string(b), time.Duration(ttl)*time.Second).Err(); err != nil {
...
func (s *RedisStore) SetWithDuration(ctx context.Context, key string, value interface{}, duration time.Duration) error {
	if s.Client == nil {
		return nil
	}
	...
	if err := s.Client.Set(ctx, key, string(b), duration).Err(); err != nil {
...
func (s *RedisStore) UpdateTTL(ctx context.Context, key string, ttl int) error {
	if s.Client == nil {
		return sdk.WithStack(fmt.Errorf("redis> cannot get redis client"))
	}
...
func (s *RedisStore) DeleteAll(ctx context.Context, pattern string) error {
	if s.Client == nil {
		return sdk.WithStack(fmt.Errorf("redis> cannot get redis client"))
	}
	keys, err := s.Client.Keys(ctx, pattern).Result()
...
func (s *RedisStore) Exist(ctx context.Context, key string) (bool, error) {
	if s.Client == nil {
		return false, sdk.WithStack(fmt.Errorf("redis> cannot get redis client"))
	}
...
func (s *RedisStore) Enqueue(ctx context.Context, queueName string, value interface{}) error {
	if s.Client == nil {
		return sdk.WithStack(fmt.Errorf("redis> cannot get redis client"))
	}
	b, err := json.Marshal(value)
	...
	if err := s.Client.LPush(ctx, queueName, string(b)).Err(); err != nil {
...
func (s *RedisStore) QueueLen(ctx context.Context, queueName string) (int, error) {
	if s.Client == nil {
		return 0, sdk.WithStack(fmt.Errorf("redis> cannot get redis client"))
	}
	var errRedis error
	var res int64
	res, errRedis = s.Client.LLen(ctx, queueName).Result()
	if errRedis != nil {
...
func (s *RedisStore) DequeueWithContext(ctx context.Context, queueName string, waitDuration time.Duration, value interface{}) error {
	if s.Client == nil {
		return sdk.WithStack(fmt.Errorf("redis> cannot get redis client"))
	}
	...
	for elem == "" {
		...
		case <-ticker.C:
			...
			res, err := s.Client.BRPop(ctx, time.Second, queueName).Result()
			if err == redis.Nil {
				continue
			}
			if err == io.EOF {
...
func (s *RedisStore) Publish(ctx context.Context, channel string, value interface{}) error {
	...
	for i := 0; i < 10; i++ {
		_, errP := s.Client.Publish(ctx, channel, iUnquoted).Result()
		if errP == nil {
			break
		}
...
func (s *RedisStore) RemoveFromQueue(ctx context.Context, rootKey string, memberKey string) error {
	if err := s.Client.LRem(ctx, rootKey, 0, memberKey).Err(); err != nil {
...
func (s *RedisStore) SetAdd(ctx context.Context, rootKey string, memberKey string, member interface{}) error {
	err := s.Client.ZAdd(ctx, rootKey, redis.Z{
		Member: memberKey,
	...
	return s.SetWithTTL(ctx, Key(rootKey, memberKey), member, -1)
}

// SetRemove removes a member from a set
func (s *RedisStore) SetRemove(ctx context.Context, rootKey string, memberKey string, _ interface{}) error {
	if err := s.Client.ZRem(ctx, rootKey, memberKey).Err(); err != nil {
		return sdk.WrapError(err, "error on SetRemove")
	}
	return s.Delete(ctx, Key(rootKey, memberKey))
}

// SetCard returns the cardinality of a ZSet
func (s *RedisStore) SetCard(ctx context.Context, key string) (int, error) {
	v := s.Client.ZCard(ctx, key)
	...
	if len(keys) > 0 {
		...
		for i := range members {
			...
			if res[i] == nil {
				...
				if err := s.Client.ZRem(ctx, key, values[i]).Err(); err != nil {
...
func (s *RedisStore) SetSearch(ctx context.Context, key, pattern string) ([]string, error) {
	keys, _, err := s.Client.ZScan(ctx, key, 0, pattern, 0).Result()
	if err != nil {
...
func (s *RedisStore) Lock(ctx context.Context, key string, expiration time.Duration, retrywdMillisecond int, retryCount int) (bool, error) {
	var errRedis error
	var res bool
	...
	for i := 0; i < retryCount; i++ {
		res, errRedis = s.Client.SetNX(ctx, key, "true", expiration).Result()
		if errRedis == nil && res {
			break
		}
...
func (s *RedisStore) Size(ctx context.Context, key string) (int64, error) {
	oct, err := s.Client.MemoryUsage(ctx, key).Result()
	return oct, sdk.WithStack(err)
}

func (s *RedisStore) ScoredSetGetScore(ctx context.Context, key string, member interface{}) (float64, error) {
	bts, err := json.Marshal(member)
	if err != nil {
		return 0, sdk.WithStack(err)
	}
	score, err := s.Client.ZScore(ctx, key, string(bts)).Result()
	return score, sdk.WithStack(err)
}

func (s *RedisStore) ScoredSetAppend(ctx context.Context, key string, value interface{}) error {
	highItem, err := s.Client.ZRevRange(ctx, key, 0, 0).Result()
	if err != nil {
		return sdk.WithStack(err)
	}
	if len(highItem) == 0 {
	...
	maxScore, err := s.Client.ZScore(ctx, key, highItem[0]).Result()
	if err != nil {
		return sdk.WithStack(err)
	}
...
func (s *RedisStore) ScoredSetRevRange(ctx context.Context, key string, offset int64, limit int64, dest interface{}) error {
	values, err := s.Client.ZRevRange(ctx, key, offset, limit).Result()
	if err != nil {
...
func (s *RedisStore) ScoredSetScanWithScores(ctx context.Context, key string, from, to float64) ([]SetValueWithScore, error) {
	...
	values, err := s.Client.ZRangeByScoreWithScores(ctx, key, &redis.ZRangeBy{
		Min: min,
		Max: max,
	}).Result()
	if err != nil {
...
func (p *RedisPubSub) GetMessage(ctx context.Context) (string, error) {
	if msg, _ := p.PubSub.ReceiveTimeout(ctx, time.Second); msg != nil {
...
func (s *RedisStore) ScoredSetScanMaxScore(ctx context.Context, key string) (*SetValueWithScore, error) {
	values, err := s.Client.ZRevRangeByScoreWithScores(ctx, key, &redis.ZRangeBy{
		Min:    "-inf",
		Max:    "+inf",
		Offset: 0,
		Count:  1,
...
func (s *RedisStore) Eval(ctx context.Context, expr string, args ...string) (string, error) {
	result, err := s.Client.Eval(ctx, expr, args).Result()
	if err != nil {
		return "", sdk.WithStack(err)
	}
