...
func AddDefaultVCSNotifications(ctx context.Context, store cache.Store, DBFunc func() *gorp.DbMap) error {
	...
	for workflowID, nodes := range nodesByWorkflowID {
		...
		if err != nil {
			log.Error(ctx, "migrate.AddDefaultVCSNotifications> cannot count workflow_notification for workflow id %d", workflowID)
			continue
		}
		if count != 0 {
			continue
		...
		tx, err := db.Begin()
		if err != nil {
			log.Error(ctx, "migrate.AddDefaultVCSNotifications> cannot begin transaction for workflow id %d : %v", workflowID, err)
			continue
		}
		...
		if err := tx.Insert(&dbNotif); err != nil {
			_ = tx.Rollback()
			log.Error(ctx, "migrate.AddDefaultVCSNotifications> Unable to insert workflow notification : %v", err)
			continue
		}
		notif.ID = dbNotif.ID
		...
		for i := range notif.NodeIDs {
			if _, err := tx.Exec(query, notif.ID, notif.NodeIDs[i]); err != nil {
				_ = tx.Rollback()
				log.Error(ctx, "migrate.AddDefaultVCSNotifications> Unable to insert associations between node %d and notification %d : %v", notif.NodeIDs[i], notif.ID, err)
				continue
			}
		}
		...
		if err := tx.Commit(); err != nil {
			_ = tx.Rollback()
			log.Error(ctx, "migrate.AddDefaultVCSNotifications> cannot commit transaction for workflow id %d : %v", workflowID, err)
		}
	}
