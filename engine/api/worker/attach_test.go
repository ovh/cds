...
func TestReleaseAllFromHatchery(t *testing.T) {
	...
	w1Consumer, err := authentication.NewConsumerWorker(context.TODO(), db, "worker-1", h1Consumer, 12)
	require.NoError(t, err)
	w2Consumer, err := authentication.NewConsumerWorker(context.TODO(), db, "worker-2", h2Consumer, 12)
	require.NoError(t, err)
