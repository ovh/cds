...
func InsertSecret(ctx context.Context, db gorp.SqlExecutor, s *sdk.WorkerModelSecret) error {
	s.ID = sdk.UUID()
	s.Created = time.Now()
...
func UpdateSecret(ctx context.Context, db gorp.SqlExecutor, s *sdk.WorkerModelSecret) error {
	dbSecret := workerModelSecret{WorkerModelSecret: *s}
