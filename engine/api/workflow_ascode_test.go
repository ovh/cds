...
func Test_WorkflowAsCodeWithNotifications(t *testing.T) {
	...
	defer ctrl.Finish()
	// The mock has been generated by mockgen: go get github.com/golang/mock/mockgen
	// If you have to regenerate this mock you just have to run, from directory $GOPATH/src/github.com/ovh/cds/engine/api/services:
...
version: v1.0`),
	...
	servicesClients.EXPECT().
		...
		DoAndReturn(
			func(ctx context.Context, method, path string, in interface{}, out interface{}, _ interface{}) (http.Header, int, error) {
				...
				assert.Equal(t, "http://lolcat.local", vcsHooks.URL)
	...
	// Then we will trigger a run of the workflow which should trigger an as-code operation
...
func Test_WorkflowAsCodeWithPermissions(t *testing.T) {
	api, db, _ := newTestAPI(t)
	...
	servicesClients.EXPECT().
		...
		DoAndReturn(
			func(ctx context.Context, method, path string, in interface{}, out interface{}, _ ...interface{}) (http.Header, int, error) {
				...
				ope.LoadFiles.Pattern = workflow.WorkflowAsCodePattern
				ope.LoadFiles.Results = map[string][]byte{
					"w-go-repo.yml": []byte(fmt.Sprintf(`
name: w-go-repo
version: v2.0
workflow:
  root:
