...
func (api *API) updateStepStatusHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		pbJob, errJob := pipeline.GetPipelineBuildJob(api.mustDB(ctx), api.Cache, buildID)
		if errJob != nil {
		...
		if err := pipeline.UpdatePipelineBuildJob(api.mustDB(ctx), pbJob); err != nil {
...
func (api *API) getPipelineBuildTriggeredHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		// Load Children
		pbs, err := pipeline.LoadPipelineBuildChildren(api.mustDB(ctx), p.ID, a.ID, buildNumber, env.ID)
		if err != nil {
...
func (api *API) deleteBuildHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		pbID, errPB := pipeline.LoadPipelineBuildID(api.mustDB(ctx), a.ID, p.ID, env.ID, buildNumber)
		if errPB != nil {
...
func (api *API) getBuildStateHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		if buildNumberS == "last" {
			lastBuildNumber, errg := pipeline.GetLastBuildNumberInTx(api.mustDB(ctx), p.ID, a.ID, env.ID)
			if errg != nil {
		...
		if withArtifacts == "true" {
			...
			pb.Artifacts, errLoadArtifact = artifact.LoadArtifactsByBuildNumber(api.mustDB(ctx), p.ID, a.ID, buildNumber, env.ID)
		...
		if withTests == "true" {
			tests, errLoadTests := pipeline.LoadTestResults(api.mustDB(ctx), pb.ID)
			if errLoadTests != nil {
...
func (api *API) addQueueResultHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		// Load Build
		pbJob, errJob := pipeline.GetPipelineBuildJob(api.mustDB(ctx), api.Cache, id)
		if errJob != nil {
...
func (api *API) getPipelineBuildJobHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		j, err := pipeline.GetPipelineBuildJob(api.mustDB(ctx), api.Cache, id)
		if err != nil {
...
func (api *API) takePipelineBuildJobHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		if caller.ModelID != 0 {
			wm, errModel := worker.LoadWorkerModelByID(api.mustDB(ctx), caller.ModelID)
			if errModel != nil {
		...
		pb, errPb := pipeline.LoadPipelineBuildByID(api.mustDB(ctx), pbJob.PipelineBuildID)
		if errPb != nil {
		...
		if errSecret := loadActionBuildSecretsAndKeys(api.mustDB(ctx), api.Cache, pbJob.ID, &pbji); errSecret != nil {
...
func (api *API) getQueueHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		case sdk.HatcheryAgent:
			queue, errQ = pipeline.LoadGroupWaitingQueue(api.mustDB(ctx), api.Cache, getHatchery(ctx).GroupID)
		case sdk.WorkerAgent:
			queue, errQ = pipeline.LoadGroupWaitingQueue(api.mustDB(ctx), api.Cache, getWorker(ctx).GroupID)
		default:
			queue, errQ = pipeline.LoadUserWaitingQueue(api.mustDB(ctx), api.Cache, getUser(ctx))
		}
...
func (api *API) addBuildVariableHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		// Check that pipeline exists
		p, errLP := pipeline.LoadPipeline(api.mustDB(ctx), projectKey, pipelineName, false)
		if errLP != nil {
			return sdk.WrapError(errLP, "addBuildVariableHandler> Cannot load pipeline %s", pipelineName)
		}

		// Check that application exists
		a, errLA := application.LoadByName(api.mustDB(ctx), api.Cache, projectKey, appName, getUser(ctx))
		if errLA != nil {
...
func (api *API) addBuildTestResultsHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		// Check that application exists
		a, errln := application.LoadByName(api.mustDB(ctx), api.Cache, projectKey, appName, getUser(ctx))
		if errln != nil {
			return sdk.WrapError(errln, "addBuildTestResultsHandler> Cannot load application %s", appName)
		}

		// load pipeline_build.id
		pb, errl := pipeline.LoadPipelineBuildByApplicationPipelineEnvBuildNumber(api.mustDB(ctx), a.ID, p.ID, env.ID, buildNumber)
		if errl != nil {
		...
		tests, err := pipeline.LoadTestResults(api.mustDB(ctx), pb.ID)
		if err != nil {
		...
		if err := pipeline.UpdateTestResults(api.mustDB(ctx), pb.ID, tests); err != nil {
...
func (api *API) getBuildTestResultsHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		if buildNumberS == "last" {
			var errlb error
			bn, errlb := pipeline.GetLastBuildNumberInTx(api.mustDB(ctx), p.ID, a.ID, env.ID)
			if errlb != nil {
		...
		// load pipeline_build.id
		pb, errlpb := pipeline.LoadPipelineBuildByApplicationPipelineEnvBuildNumber(api.mustDB(ctx), a.ID, p.ID, env.ID, buildNumber)
		if errlpb != nil {
			return sdk.WrapError(errlpb, "getBuildTestResultsHandler> Cannot load pipeline build")
		}

		tests, errltr := pipeline.LoadTestResults(api.mustDB(ctx), pb.ID)
		if errltr != nil {
