...
func (api *API) updateGroupRoleOnApplicationHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		app, errload := application.LoadByName(api.mustDB(ctx), api.Cache, key, appName, getUser(ctx), application.LoadOptions.WithGroups)
		if errload != nil {
			return sdk.WrapError(errload, "updateGroupRoleOnApplicationHandler: Cannot load application %s", appName)
		}

		g, errLoadGroup := group.LoadGroup(api.mustDB(ctx), groupName)
		if errLoadGroup != nil {
...
func (api *API) addGroupInApplicationHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		proj, err := project.Load(api.mustDB(ctx), api.Cache, key, getUser(ctx))
		if err != nil {
...
func (api *API) deleteGroupFromApplicationHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		app, err := application.LoadByName(api.mustDB(ctx), api.Cache, key, appName, getUser(ctx), application.LoadOptions.WithGroups)
		if err != nil {
