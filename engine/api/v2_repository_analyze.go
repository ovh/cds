...
func (api *API) analyzeRepository(ctx context.Context, projectRepoID string, analysisID string) error {
	...
	ef, err := NewEntityFinder(ctx, api.mustDB(), proj.Key, analysis.Ref, analysis.Commit, *repo, *vcsProjectWithSecret, *u, analysis.Data.CDSAdminWithMFA, api.Config.WorkflowV2.LibraryProjectKey)
	if err != nil {
		return err
	}
