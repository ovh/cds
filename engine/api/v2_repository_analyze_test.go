...
func TestAnalyzeWorkflowWithProjectConcurrency(t *testing.T) {
	api, db, _ := newTestAPI(t)
	ctx := context.TODO()
	...
	require.NoError(t, project.InsertConcurrency(ctx, db, &pc))
...
jobs:
  root:
	...
	servicesClients.EXPECT().DoJSONRequest(gomock.Any(), "GET", "/vcs/vcs-server/repos/my/repo/branches/?branch=&default=true&noCache=true", gomock.Any(), gomock.Any(), gomock.Any()).Times(1).
		DoAndReturn(
			func(ctx context.Context, method, path string, in interface{}, out interface{}, _ interface{}) (http.Header, int, error) {
				contents := sdk.VCSBranch{
					ID: "refs/heads/master",
...
func TestAnalyzeWorkflowWithConcurrency(t *testing.T) {
	api, db, _ := newTestAPI(t)
	ctx := context.TODO()
...
func TestAnalyzeWorkflowWithConcurrencyNotFound(t *testing.T) {
	api, db, _ := newTestAPI(t)
	ctx := context.TODO()
