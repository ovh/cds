...
func (api *API) addPollerHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		app, err := application.LoadByName(api.mustDB(ctx), api.Cache, projectKey, appName, getUser(ctx), application.LoadOptions.Default)
		if err != nil {
			return sdk.WrapError(err, "addPollerHandler> Cannot load application")
		}

		app.RepositoryPollers, err = poller.LoadByApplication(api.mustDB(ctx), app.ID)
		if err != nil {
		...
		err = poller.Insert(api.mustDB(ctx), &h)
		if err != nil {
...
func (api *API) getApplicationPollersHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		a.RepositoryPollers, err = poller.LoadByApplication(api.mustDB(ctx), a.ID)
		if err != nil {
...
func (api *API) getPollersHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		poller, err := poller.LoadByApplicationAndPipeline(api.mustDB(ctx), a.ID, p.ID)
		if err != nil {
...
func (api *API) deletePollerHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		po, err := poller.LoadByApplicationAndPipeline(api.mustDB(ctx), a.ID, p.ID)
		if err != nil {
		...
		var errW error
		a.Workflows, errW = workflowv0.LoadCDTree(api.mustDB(ctx), api.Cache, projectKey, appName, getUser(ctx), "", "", 0)
		if errW != nil {
