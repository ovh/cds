...
func (api *API) authMiddleware(ctx context.Context, w http.ResponseWriter, req *http.Request, rc *service.HandlerConfig) (context.Context, error) {
	...
	if ok {
		...
		log.Debug("authMiddleware> try to retrieve session for given jwt with id: %d", sessionID)
	...
	if session != nil {
		...
		log.Debug("authMiddleware> try to retrieve consumer for given session with id: %d", session.ConsumerID)
