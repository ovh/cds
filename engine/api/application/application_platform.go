...
func LoadDeploymentStrategies(db gorp.SqlExecutor, appID int64, withClearPassword bool) (map[string]sdk.PlatformConfig, error) {
	query := `SELECT platform_model.name, application_deployment_strategy.config
...
// SetDeploymentStrategies update the application_deployment_strategy table
func SetDeploymentStrategies(db gorp.SqlExecutor, projID, appID, pfID int64, cfg sdk.PlatformConfig) error {
	...
	for k, v := range cfg {
		if v.Type == sdk.PlatformConfigTypePassword {
			...
			if err != nil {
				return sdk.WrapError(err, "SetDeploymentStrategies> unable to encrypt data")
			}
			newcfg[k] = sdk.PlatformConfigValue{
				Type:  sdk.PlatformConfigTypePassword,
	...
	if err != nil {
		return sdk.WrapError(err, "SetDeploymentStrategies> unable to check if deployment strategy exist")
	}

	scfg, err := gorpmapping.JSONToNullString(newcfg)
	if err != nil {
		return sdk.WrapError(err, "SetDeploymentStrategies> unable to parse deployment strategy ")
	}
	...
	if count == 1 {
		...
		if _, err := db.Exec(query, scfg, appID, projID, pfID); err != nil {
			return sdk.WrapError(err, "SetDeploymentStrategies> unable to update deployment strategy")
		}
		return nil
	}
