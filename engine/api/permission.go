...
func (api *API) checkWorkerPermission(ctx context.Context, db gorp.SqlExecutor, rc *service.HandlerConfig, routeVar map[string]string) bool {
	if getWorker(ctx) == nil {
		log.Error(ctx, "checkWorkerPermission> no worker in ctx")
		return false
	}
	...
	if rc.Options["isExecution"] == "true" {
		...
		if !ok {
			log.Error(ctx, "checkWorkerPermission> actionBuildID:%v runNodeJob.ID:%v", getWorker(ctx).ActionBuildID, runNodeJob.ID)
		}
		return ok
	}
	return true
