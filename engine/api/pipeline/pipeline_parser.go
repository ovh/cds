...
func ParseAndImport(ctx context.Context, db gorp.SqlExecutor, projIdent sdk.ProjectIdentifiers, projPermissions []sdk.GroupPermission, epip exportentities.Pipeliner, u sdk.Identifiable, opts ImportOptions) (*sdk.Pipeline, []sdk.Message, error) {
	...
	exist, errE := ExistPipeline(db, projIdent.ID, pip.Name)
	if errE != nil {
	...
	previousPip := pip
	if exist {
		prevPip, err := LoadPipeline(ctx, db, projIdent.Key, pip.Name, true)
		if err != nil {
	...
	if exist && !opts.Force {
		...
		globalError = ImportUpdate(ctx, db, projIdent, projPermissions, pip, msgChan)
	} else {
		globalError = Import(ctx, db, projIdent, projPermissions, pip, msgChan, u)
	}
