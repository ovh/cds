...
func Test_deleteGroupFromApplicationHandler(t *testing.T) {
	...
	test.NoError(t, group.InsertGroup(db, &gr2))

	if u.Groups == nil {
		u.Groups = []sdk.Group{gr, gr2}
	} else {
		u.Groups = append(u.Groups, gr, gr2)
	}
	...
	test.NoError(t, group.InsertGroupInApplication(db, app.ID, gr.ID, 7))
	test.NoError(t, group.InsertGroupInApplication(db, app.ID, gr2.ID, 7))
