...
func CreateFromRepository(ctx context.Context, db *gorp.DbMap, store cache.Store, projIdent sdk.ProjectIdentifiers, projPushData sdk.ProjectForWorkflowPush, projKeys []sdk.ProjectKey, vcsServers []sdk.ProjectVCSServerLink, wf *sdk.Workflow,
	...
	log.Info(ctx, "polling operation %v for workflow %s/%s", newOperation.UUID, projIdent.Key, wf.Name)
	...
	if err != nil {
		...
		log.ErrorWithFields(ctx, fields, "cannot analyse repository (operation %s for workflow %s/%s): %v", newOperation.UUID, projIdent.Key, wf.Name, err)
