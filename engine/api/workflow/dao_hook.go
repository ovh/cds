...
// LoadAndLockHookByUUID load hook given his uuid and lock it in database
func LoadAndLockHookByUUID(db gorp.SqlExecutor, uuid string) (*sdk.WorkflowNodeHook, error) {
	query := `
		SELECT id, uuid, workflow_hook_model_id, workflow_node_id
			FROM workflow_node_hook
		WHERE uuid = $1 FOR UPDATE NOWAIT`
