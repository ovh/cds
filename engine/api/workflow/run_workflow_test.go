...
func TestManualRun1(t *testing.T) {
	...
	//TestLoadNodeRun
	nodeRun, err := workflow.LoadNodeRun(db, proj.Key, "test_1", lastrun.WorkflowNodeRuns[w1.WorkflowData.Node.ID][0].ID, workflow.LoadRunOptions{})
	require.NoError(t, err)
...
queueRun:
	...
	for i := range jobs {
		...
		//BookNodeJobRun
		_, err = workflow.BookNodeJobRun(context.TODO(), cache, 120, j.ID, &sdk.Service{
