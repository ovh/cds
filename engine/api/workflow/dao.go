...
func deleteOrUnlinkDependencies(ctx context.Context, db gorpmapper.SqlExecutorWithTx, cache cache.Store, proj sdk.Project, wf *sdk.Workflow) error {
	...
	for appID := range wf.Applications {
		...
		if app.FromRepository == wf.FromRepository && len(otherWfs) > 0 {
			...
			app.FromRepository = ""
			if err := application.Update(ctx, db, &app); err != nil {
				return err
			}
...
func checkProjectIntegration(proj sdk.Project, w *sdk.Workflow, n *sdk.Node) error {
	...
	if n.Context.ProjectIntegrationName != "" {
		...
		if ppProj.ID == 0 {
			return sdk.NewErrorFrom(sdk.ErrIntegrationNotFound, "unknown integration %s", n.Context.ProjectIntegrationName)
		}
...
func checkIntegration(proj sdk.Project, w *sdk.Workflow) error {
	...
	for i := range w.Integrations {
		...
		if !found {
			return sdk.NewErrorFrom(sdk.ErrIntegrationNotFound, "unknown integration %s", workflowIntegration.ProjectIntegration.Name)
		}
...
func checkApplication(ctx context.Context, db gorp.SqlExecutor, proj sdk.Project, w *sdk.Workflow, n *sdk.Node) error {
	if n.Context.ApplicationID != 0 {
		...
		if !ok {
			appDB, err := application.LoadByID(ctx, db, n.Context.ApplicationID, application.LoadOptions.WithDeploymentStrategies, application.LoadOptions.WithVariables, application.LoadOptions.WithKeys)
			if err != nil {
				return err
			}
			app = *appDB
	...
	if n.Context.ApplicationName != "" {
		appDB, err := application.LoadByName(ctx, db, proj.Key, n.Context.ApplicationName, application.LoadOptions.WithDeploymentStrategies, application.LoadOptions.WithVariables, application.LoadOptions.WithKeys)
		if err != nil {
...
func Push(ctx context.Context, db *gorp.DbMap, store cache.Store, proj *sdk.Project, data exportentities.WorkflowComponents,
	...
	wf, msgList, err := ParseAndImport(ctx, tx, store, *proj, oldWf, data.Workflow, consumer, importOptions)
	allMsg = append(allMsg, msgList...)
	if err != nil {
