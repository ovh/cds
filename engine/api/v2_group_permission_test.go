...
func Test_getGroupPermissionHandler_noPermissions(t *testing.T) {
	...
	uri := api.Router.GetRouteV2("GET", api.getGroupPermissionHandler, vars)
	test.NotEmpty(t, uri)
...
func Test_getGroupPermissionHandler_withGlobalRole(t *testing.T) {
	api, db, _ := newTestAPI(t)
...
func Test_getGroupPermissionHandler_withProjectRole(t *testing.T) {
	api, db, _ := newTestAPI(t)
...
func Test_getGroupPermissionHandler_unknownGroup(t *testing.T) {
	api, db, _ := newTestAPI(t)

	u, pass := assets.InsertAdminUser(t, db)

	vars := map[string]string{"name": "unknown-group-does-not-exist"}
	uri := api.Router.GetRouteV2("GET", api.getGroupPermissionHandler, vars)
	test.NotEmpty(t, uri)
