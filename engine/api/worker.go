	...
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		if !isAdmin(ctx) {
			...
			if wk.HatcheryID == nil {
				return sdk.WrapError(sdk.ErrForbidden, "hatchery %d cannot disable worker %d started by %s that is no more linked to an hatchery", hatcherySrv.ID, wk.ID, wk.HatcheryName)
			}
