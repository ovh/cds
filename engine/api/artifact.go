...
func (api *API) uploadArtifactHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		p, errP := pipeline.LoadPipeline(api.mustDB(ctx), key, pipelineName, false)
		if errP != nil {
			return sdk.WrapError(errP, "uploadArtifactHandler> cannot load pipeline %s-%s", key, pipelineName)
		}

		a, errA := application.LoadByName(api.mustDB(ctx), api.Cache, key, appName, getUser(ctx))
		if errA != nil {
		...
		for i := range files {
			...
			if err := artifact.SaveFile(api.mustDB(ctx), p, a, art, file, env); err != nil {
				file.Close()
...
func (api *API) downloadArtifactHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		// Load artifact
		art, err := artifact.LoadArtifact(api.mustDB(ctx), int64(artifactID))
		if err != nil {
...
func (api *API) listArtifactsBuildHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		art, errArt := artifact.LoadArtifactsByBuildNumber(api.mustDB(ctx), p.ID, a.ID, buildNumber, env.ID)
		if errArt != nil {
...
func (api *API) listArtifactsHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		art, errArt := artifact.LoadArtifacts(api.mustDB(ctx), p.ID, a.ID, env.ID, tag)
		if errArt != nil {
...
func (api *API) downloadArtifactDirectHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		art, err := artifact.LoadArtifactByHash(api.mustDB(ctx), hash)
		if err != nil {
...
func (api *API) postArtifactWithTempURLHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		if envName == "" || envName == sdk.DefaultEnv.Name {
			...
			var errE error
			env, errE = environment.LoadEnvironmentByName(api.mustDB(ctx), proj, envName)
			if errE != nil {
...
func (api *API) postArtifactWithTempURLCallbackHandler() Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		if envName == "" || envName == sdk.DefaultEnv.Name {
			...
			var errE error
			env, errE = environment.LoadEnvironmentByName(api.mustDB(ctx), projKey, envName)
			if errE != nil {
		...
		pip, errpip := pipeline.LoadPipeline(api.mustDB(ctx), projKey, pipName, false)
		if errpip != nil {
			return sdk.WrapError(errpip, "postArtifactWithTempURLCallbackHandler> Unable to load pipeline %s/%s", projKey, pipName)
		}

		app, errapp := application.LoadByName(api.mustDB(ctx), api.Cache, projKey, appName, getUser(ctx))
		if errapp != nil {
			return sdk.WrapError(errapp, "postArtifactWithTempURLCallbackHandler> Unable to load application %s/%s", projKey, appName)
		}

		if err := artifact.InsertArtifact(api.mustDB(ctx), pip.ID, app.ID, env.ID, *art); err != nil {
