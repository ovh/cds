...
func TestRbacProjectManageNotification(t *testing.T) {
...
- role: %s
  ...
  users: [%s]`
	...
	rbacYaml = fmt.Sprintf(rbacYaml, sdk.RandomString(10), sdk.ProjectRoleManageNotification, p.Key, user1.Username)
	var r sdk.RBAC
...
func TestRbacProjectManage(t *testing.T) {
	api, db, _ := newTestAPI(t)
...
- role: %s
  ...
  users: [%s]`
	...
	rbacYaml = fmt.Sprintf(rbacYaml, sdk.RandomString(10), sdk.ProjectRoleManage, p.Key, user1.Username)
	var r sdk.RBAC
	require.NoError(t, yaml.Unmarshal([]byte(rbacYaml), &r))
...
func TestRbacProjectRead(t *testing.T) {
	api, db, _ := newTestAPI(t)
