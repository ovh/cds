...
func (api *API) postApplicationImportHandler() service.Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		//Load project
		proj, errp := project.Load(api.mustDB(), api.Cache, key, getAuthentifiedUser(ctx), project.LoadOptions.WithGroups, project.LoadOptions.WithIntegrations)
		if errp != nil {
		...
		newApp, msgList, globalError := application.ParseAndImport(tx, api.Cache, proj, eapp, application.ImportOptions{Force: force}, project.DecryptWithBuiltinKey, getAuthentifiedUser(ctx))
		...
		event.PublishAddApplication(proj.Key, *newApp, getAuthentifiedUser(ctx))
