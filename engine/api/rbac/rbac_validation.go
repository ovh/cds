...
func isValidRBACProject(rbacName string, rbacProject sdk.RBACProject) error {
  ...
  if len(rbacProject.RBACProjectKeys) == 0 && !rbacProject.All {
    return sdk.NewErrorFrom(sdk.ErrInvalidData, "rbac %s: must have at least 1 project on a project permission", rbacName)
  }
  if len(rbacProject.RBACProjectKeys) > 0 && rbacProject.All {
    return sdk.NewErrorFrom(sdk.ErrInvalidData, "rbac %s: you can't have a list of project and the all flag checked on a project permission", rbacName)
  }
  return nil
}
