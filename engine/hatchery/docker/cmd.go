...
$ hatchery docker --api=https://<api.domain> --token=<token>
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		hatchery.Create(hatcheryDocker, viper.GetString("api"), viper.GetString("token"), viper.GetInt("provision"), viper.GetInt("request-api-timeout"), viper.GetInt("max-failures-heartbeat"), viper.GetBool("insecure"), viper.GetInt("spawn-threshold-warning"), viper.GetInt("spawn-threshold-critical"), viper.GetInt("grace-time-queued"))
	},
}
