...
func TestSpawnWorkerTimeout(t *testing.T) {
	...
	t.Logf("%+v\n", err)
	assert.Error(t, err)
	assert.Equal(t, "spawnMarathonDockerWorker> TestSpawnWorkerTimeout>SpawnWorker>Append: internal server error (caused by: deployment for aaa timeout)", err.Error())
}
