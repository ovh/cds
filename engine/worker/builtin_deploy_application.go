	...
	return func(ctx context.Context, a *sdk.Action, buildID int64, params *[]sdk.Parameter, secrets []sdk.Variable, sendLog LoggerFunc) sdk.Result {
		...
		if err != nil {
			...
			if _, err := platformPluginClient.Stop(ctx, new(empty.Empty)); err != nil {
				log.Info("Error on platformPluginClient.Stop (manifest): %s", err)
			}
			stopLogs()
			<-done
			sendLog(err.Error())
		...
		if err != nil {
			...
			if _, err := platformPluginClient.Stop(ctx, new(empty.Empty)); err != nil {
				log.Info("Error on platformPluginClient.Stop (Deploy): %s", err)
			}
			sendLog(res.Reason)
			stopLogs()
			<-done
		...
		if strings.ToUpper(res.Status) == strings.ToUpper(sdk.StatusSuccess.String()) {
			if _, err := platformPluginClient.Stop(ctx, new(empty.Empty)); err != nil {
				log.Info("Error on platformPluginClient.Stop (success): %s", err)
			}
			stopLogs()
			<-done
			return sdk.Result{
		...
		if _, err := platformPluginClient.Stop(ctx, new(empty.Empty)); err != nil {
			log.Info("Error on platformPluginClient.Stop (end): %s", err)
		}
