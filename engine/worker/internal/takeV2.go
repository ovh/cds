...
// signingKeyLength is the length of the key generated by the API for a worker.
const signingKeyLength = 32

// decodeSigningKey decodes the key used to sign every log line sent to the CDN. It must be
// decoded at its own length: decoding into a fixed size buffer silently keeps the trailing zeros
// of the buffer when the key is shorter, and the CDN then rejects every line of the job with an
// error in cryptographic primitive.
func decodeSigningKey(ctx context.Context, signingKey string) ([]byte, error) {
	secretKey, err := base64.StdEncoding.DecodeString(signingKey)
	if err != nil {
		return nil, sdk.WithStack(err)
	}
	if len(secretKey) == 0 {
		return nil, sdk.WithStack(fmt.Errorf("empty signing key: the job logs would all be rejected by the CDN"))
	}
	if len(secretKey) != signingKeyLength {
		log.Warn(ctx, "decodeSigningKey> signing key has an unexpected length: %d (expected %d)", len(secretKey), signingKeyLength)
	}
