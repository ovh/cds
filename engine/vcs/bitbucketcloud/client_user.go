...
func (client *bitbucketcloudClient) CurrentUser(ctx context.Context) (User, error) {
	var user User
	url := "/user"
	cacheKey := cache.Key("vcs", "bitbucketcloud", "users", client.OAuthToken+client.username, url)
...
func (client *bitbucketcloudClient) Workspaces(ctx context.Context) (Workspaces, error) {
	...
	cacheKey := cache.Key("vcs", "bitbucketcloud", "users", "workspaces", client.OAuthToken+client.username, url)
