...
func getNewConsumer(t *testing.T) sdk.VCSServer {
	...
	ghConsummer := New(consumerKey, []byte(consumerPrivateKey), url, "", "", cache, true, true)
	return ghConsummer
}
...
func newAuthorizedClient(t *testing.T) (string, string) {
	...
	ghConsummer := New(consumerKey, []byte(privateKey), url, "", "", cache, true, true)
...
func getAuthorizedClient(t *testing.T) sdk.VCSAuthorizedClient {
	...
	consumer := New(consumerKey, []byte(privateKey), url, "", "", cache, true, true)
