...
func TestCleanStorageUnit(t *testing.T) {
	m := gorpmapper.New()
	item.InitDBMapping(m)
	storage.InitDBMapping(m)
	...
	for i := 0; i < 10; i++ {
		...
		require.NoError(t, item.Insert(ctx, s.Mapper, db, &it))
