...
func (s *Service) initMetrics(ctx context.Context) error {
	...
	s.Metrics.metricsWSClients = stats.Int64("cdn/websocket_clients", "number of  websocket clients", stats.UnitDimensionless)
	metricsWSClients := telemetry.NewViewCount(s.Metrics.metricsWSClients.Name(), s.Metrics.metricsWSClients, []tag.Key{tagServiceName, tagItemType})

	s.Metrics.metricsWSEvents = stats.Int64("cdn/websocket_events", "number of websocket events", stats.UnitDimensionless)
	metricsWSEvents := telemetry.NewViewCount(s.Metrics.metricsWSEvents.Name(), s.Metrics.metricsWSEvents, []tag.Key{tagServiceName, tagItemType})
