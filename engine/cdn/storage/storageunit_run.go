...
func (x *RunningStorageUnits) Run(ctx context.Context, s StorageUnit, syncMinNbElements, nbItem int64) error {
	if _, err := LoadUnitByID(ctx, x.m, x.db, s.ID()); err != nil {
		return err
	}

	// Load items to sync
	itemIDs, err := LoadAllItemIDUnknownByUnit(x.db, s.ID(), syncMinNbElements, nbItem)
	if err != nil {
		return err
	}
