...
func (s *Service) RunTcpLogServer(ctx context.Context) {
	...
	for i := int64(0); i <= s.Cfg.NbJobLogsGoroutines; i++ {
		s.GoRoutines.Loop(ctx, fmt.Sprintf("cdn-worker-job-%d", i), func(ctx context.Context) {
	...
	for i := int64(0); i < s.Cfg.NbServiceLogsGoroutines; i++ {
		s.GoRoutines.Loop(ctx, fmt.Sprintf("cdn-worker-service-%d", i), func(ctx context.Context) {
	...
	// DEPRECATED
	s.GoRoutines.Loop(ctx, "cdn-waiting-job", func(ctx context.Context) {
		s.waitingJobs(ctx)
	})
	...
	go func() {
		for {
			...
			s.GoRoutines.Run(ctx, "cdn-logServer", func(ctx context.Context) {
