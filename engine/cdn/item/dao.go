...
// MarkItemsAsToDelete marks a list of items as to_delete by their IDs.
func MarkItemsAsToDelete(db gorpmapper.SqlExecutorWithTx, ids []string) error {
	query := `UPDATE item SET to_delete = true WHERE id = ANY($1)`
	_, err := db.Exec(query, pq.StringArray(ids))
	return sdk.WithStack(err)
}
