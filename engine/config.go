		...
		case sdk.TypeCDN:
			...
			conf.CDN.Units.Storages = []storage.StorageConfiguration{
				...
				},
				{
					...
					SyncBandwidth: 10,
					SyncParallel:  2,
					CDS: &storage.CDSStorageConfiguration{ // Token will ge generated by configSetStartupData func
...
func configSetStartupData(conf *Configuration) (string, error) {
	...
	if conf.CDN != nil {
		...
		for i := 0; i < len(conf.CDN.Units.Storages); i++ {
			if conf.CDN.Units.Storages[i].CDS != nil {
				var cfg = api.StartupConfigConsumer{
					ID:          sdk.UUID(),
					Name:        "cdn",
				...
				conf.CDN.Units.Storages[i].CDS.Token, err = builtin.NewSigninConsumerToken(&c)
				if err != nil {
					return "", err
				}
				startupCfg.Consumers = append(startupCfg.Consumers, cfg)
...
func getInitTokenFromExistingConfiguration(conf Configuration) (string, error) {
	...
	if conf.CDN != nil {
		...
		for i := 0; i < len(conf.CDN.Units.Storages); i++ {
			if conf.CDN.Units.Storages[i].CDS != nil {
				consumerID, iat, err := builtin.CheckSigninConsumerToken(conf.CDN.Units.Storages[i].CDS.Token)
				if err != nil {
					return "", fmt.Errorf("cannot parse cdn token: %v", err)
				}
				if iat < globalIAT {
